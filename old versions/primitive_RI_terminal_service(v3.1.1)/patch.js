(function() {
    ...
    function initClock() {
        ...
        // CHANGED COLOR HERE: #E0E0E0 (Light Gray)
    ...
    function showToast(message, type = 'info', title = 'SYSTEM') {
        ...
        toast.innerHTML = `
            <span class="rhodes-toast-header">// ${title}:</span>
            <span class="rhodes-toast-msg">${message}</span>
        `;
    ...
    function requestData() {
        ...
        setTimeout(() => showToast("Connecting to Neural Network...", "info", "NETWORK"), 200);

        if (!chrome || !chrome.runtime || !chrome.runtime.sendMessage) {
            showToast("Runtime Detached", "error", "ERROR");
        ...
        try {
            chrome.runtime.sendMessage({ type: 'FETCH_NEWS' }, (response) => {
                if (chrome.runtime.lastError || !response) {
                    showToast("Connection Failed", "error", "OFFLINE");
                    showToast("Loading Local Cache", "error", "BACKUP");
                    renderData(SAFE_DATA, newsContainer, highlightsContainer);
                } else {
                    if (response.isOffline) {
                        showToast("Live Data Unreachable", "error", "WARNING");
                        showToast("Restoring Backup Data", "info", "SYSTEM");
                    } else {
                        showToast("Connection Established", "success", "ONLINE");
                    }
