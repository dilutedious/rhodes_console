(function() {
    ...
    const SAFE_DATA = {
        isOffline: true,
        highlights: [{ title: "Welcome Doctor", subtitle: "SYSTEM", cover: "https://web.hycdn.cn/arknights/official/assets/images/bg.jpg", link: "#" }],
        newsList: [{ title: "Initializing...", date: "...", link: "#" }]
    };
    ...
    function startCarousel(container, items) {
        ...
        const updateSlide = () => {
            ...
            // Preload image to check if it exists (fallback if 404)
        ...
        // Auto-Play (only if more than 1 item)
        if (items.length > 1) {
    ...
    function renderData(data, newsContainer, highlightsContainer) {
        ...
        if (data.isOffline) {
            ...
            badge.style.cssText = "background:#ffc107; color:black; font-weight:bold; padding:4px; text-align:center; font-size:0.7rem; margin-bottom:10px; border-radius: 2px;";
        ...
        (data.newsList || []).forEach(item => {
            ...
            const dateColor = item.date.includes("LIVE") ? "#29B6F6" : "#aaa";
